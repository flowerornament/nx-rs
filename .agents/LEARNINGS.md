...
3. System-command integration fixtures now live under `tests/fixtures/system/`.
- The system matrix base repo path is `tests/fixtures/system/repo_base`.
- `tests/system_command_matrix.rs` expects that fixture path directly.
...
6. Shared integration-test helpers are centralized in `tests/support.rs`.
- Keep filesystem copy/snapshot helpers there rather than duplicating in individual integration tests.
...
10. Deterministic behavioral coverage now centers on system-command matrix + unit tests.
- `tests/system_command_matrix.rs` is the canonical end-to-end stubbed command contract suite.
- Matrix coverage includes rebuild/update/upgrade/install/remove/info/query/undo behavioral contracts under deterministic stubs.
- Verified on 2026-02-28 with `just ci`, `just test-system`, and `just cutover-validate`.
...
- Verified on 2026-02-19 with `just ci`, `just test-system`, and `just cutover-validate`.
...
- Verified on 2026-02-20 with `just ci`, `just test-system`, and `just cutover-validate`.
...
- Verified on 2026-02-27 with `just ci`, `just test-system`, and `just cutover-validate`.
...
- `just test-system` and `just cutover-validate` remain available for ad hoc forensic/recovery validation only.
- Historical maintenance-gate artifacts remain preserved under `.agents/reports/maintenance-gates/`.
- Verified on 2026-02-28 with `just ci`, `cargo clippy --workspace --all-targets --all-features -- -D warnings -W clippy::pedantic`, `just test-system`, and `just cutover-validate`.

30. Parity harness and parity fixture corpus were removed from the repo.
- Deleted `tests/parity_harness.rs` and `tests/fixtures/parity/` to reduce maintenance surface.
- Operational verification now uses `just ci`, `just test-system`, and `just cutover-validate`.
- Rollback guidance remains flake-revision pinning for `nx-rs`, not Python restoration.
- Verified on 2026-02-28 with `just ci`, `cargo clippy --workspace --all-targets --all-features -- -D warnings -W clippy::pedantic`, `just test-system`, and `just cutover-validate`.
