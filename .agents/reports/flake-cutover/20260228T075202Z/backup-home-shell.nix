...

{
  ...
  # Order: user paths → Nix paths → Homebrew → system paths
  home.sessionPath = [
    "$HOME/.local/bin"
  ...
  programs.zsh = {
    ...
    envExtra = ''
      ...
      # Bridge sops secret names to tool-specific env vars
      export TORRENT_GETTER_REDACTED_API_KEY="''${REDACTED_API_KEY:-}"
