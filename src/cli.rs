...
const DOCTOR_HELP: &str = "Examples:\n  nx doctor\n  nx doctor --verbose\n  nx doctor --json\n\nNotes:\n  - `doctor` is repo-scoped and expects to run inside a managed nix config repo.\n  - Reports repo checks and public Nix substrate health, including Determinate freshness, lazy trees, garbage collection, and observed `/nix` filesystem availability.\n  - Reserves substrate warnings for actionable conditions; optional settings and ordinary disk availability are informational.\n  - Read-only: never upgrades Nix, clears private caches, or runs garbage collection.";
...
const CLEAN_CACHES_HELP: &str = concat!(
    ...
    "\n",
    "Notes:\n",
    "  - Scans known home cache directories, reports sizes, and cleans them.\n",
    "  - Covers: cargo, uv, npm, Homebrew, Xcode, Rust targets, Elixir _build, node_modules, and selected AI agent caches.\n",
    "  - Project build artifacts are excluded by default; select rust-targets, elixir-builds, or node-modules explicitly.\n",
    "  - Nix store GC is excluded by default because it can force future downloads or local source builds.\n",
    "  - Select nix-gc explicitly when you intentionally want Nix store garbage collection.\n",
...
pub struct RebuildArgs {
    ...
    pub timing: bool,
    #[arg(long, short = 'v', help = "Stream full Nix build logs")]
    pub verbose: bool,
    #[arg(
        last = true,
...
pub struct UpgradeArgs {
    ...
    #[arg(
        long,
        short = 'y',
        conflicts_with = "allow_source_builds",
        help = "Approve a recognized source-build plan without prompting"
    )]
    pub yes: bool,
    #[arg(
        long,
...
mod tests {
    ...
    fn upgrade_exposes_spec_options_via_metadata() {
        ...
        let expected_shorts: BTreeSet<_> = ['n', 'v', 'y'].into_iter().collect();
        assert_eq!(
