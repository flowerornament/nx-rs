...
const NIX_STUB_SCRIPT: &str = include_str!("fixtures/parity/stubs/nix_stub.sh");
...
fn parity_harness() -> Result<(), Box<dyn Error>> {
    ...
    let reference_cli = if target == HarnessTarget::Python {
        Some(
            env::var_os("NX_PYTHON_CLI")
    ...
    for case in cases {
        ...
        if !baseline_path.exists() {
            return Err(io::Error::new(
                ...
                format!(
                    "missing baseline for case '{}': {}. Run `NX_PARITY_CAPTURE=1 NX_PARITY_TARGET=rust cargo test --test parity_harness -- --nocapture`.",
                    case.id,
...
fn run_case(
    ...
    match target {
        HarnessTarget::Python => {
            let cli =
                reference_cli.ok_or_else(|| io::Error::other("missing python parity binary"))?;
...
fn write_nix_stub(stub_bin: &Path) -> Result<(), Box<dyn Error>> {
    support::write_executable(&stub_bin.join("nix"), NIX_STUB_SCRIPT)?;
    Ok(())
}
